...
Daftar objek pesan notifikasi dikembalikan dalam format array terurut dari waktu pengiriman terbaru. Data tersebut kemudian ditandai di tingkat antarmuka pengguna setelah kotak masuk dibuka.
...
Penyimpanan token FCM baru ke dalam database peladen berhasil dijalankan setelah data divalidasi. Token ini akan digunakan sebagai alamat tujuan pengiriman notifikasi langsung (*push notifications*) ketika terdapat pengerjaan baru.
...
Penghapusan relasi token dari data profil pengguna diselesaikan secara langsung di database peladen. Konfirmasi penghapusan dikirimkan sebagai tanda bahwa perangkat tersebut tidak akan lagi menerima notifikasi sistem.
