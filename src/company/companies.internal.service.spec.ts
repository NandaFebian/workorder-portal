...
describe('CompaniesInternalService', () => {
    ...
    describe('inviteEmployees()', () => {
        ...
        it('UT-CMP-001: should invite single employee successfully', async () => {
            // Arrange
            jest.spyOn(service, 'findInternalById').mockResolvedValue(mockCompany as any);
            jest.spyOn(usersService, 'findOneByEmail').mockResolvedValue(mockUser as any);
            jest.spyOn(positionsService, 'findById').mockResolvedValue(mockPosition as any);
            ...
            // Act
            const result = await service.inviteEmployees('507f1f77bcf86cd799439012', inviteDto);

            // Assert
            expect(service.findInternalById).toHaveBeenCalledWith('507f1f77bcf86cd799439012');
            expect(usersService.findOneByEmail).toHaveBeenCalledWith('user@test.com');
            expect(positionsService.findById).toHaveBeenCalledWith('507f1f77bcf86cd799439050');
        ...
        it('UT-CMP-002: should invite multiple employees in bulk', async () => {
            ...
            const inviteDto = {
                invites: [
                    { email: 'user1@test.com', role: Role.CompanyStaff, positionId: '507f1f77bcf86cd799439050' },
                    { email: 'user2@test.com', role: Role.CompanyStaff, positionId: '507f1f77bcf86cd799439050' },
                    { email: 'user3@test.com', role: Role.CompanyStaff, positionId: '507f1f77bcf86cd799439050' },
                    { email: 'user4@test.com', role: Role.CompanyStaff, positionId: '507f1f77bcf86cd799439050' },
                    { email: 'user5@test.com', role: Role.CompanyStaff, positionId: '507f1f77bcf86cd799439050' },
                ],
            };
        ...
        it('UT-CMP-003: should set expiration date correctly', async () => {
            ...
            const createSpy = jest.spyOn(invitationModel, 'create').mockResolvedValue({});
            ...
            const timeDiff = Math.abs(createCall.expiresAt.getTime() - expectedTime.getTime());
        ...
        it('UT-CMP-005: should handle duplicate email gracefully', async () => {
            // Arrange
            const userWithCompany = { ...mockUser, companyId: '507f1f77bcf86cd799439099' };
            jest.spyOn(service, 'findInternalById').mockResolvedValue(mockCompany as any);
            jest.spyOn(usersService, 'findOneByEmail').mockResolvedValue(userWithCompany as any);
            jest.spyOn(positionsService, 'findById').mockResolvedValue(mockPosition as any);
            ...
            expect(result.data.invited).toHaveLength(0);
            expect(result.errors!).toHaveLength(1);
            expect(result.errors![0].message).toContain('already belongs to a company');
        });
