...
export class DatabaseSeederService {
  ...
  async seedCompanies(): Promise<void> {
    ...
    if (existingCount > 0) {
      this.logger.warn(
        `   ⚠️  ${existingCount} companies already exist, skipping...`,
      );
      return;
    }
    ...
    this.logger.log(
      `   ✅ Seeded ${SeedDataConfig.companies.length} companies`,
    );
  ...
  async seedPositions(): Promise<void> {
    ...
    if (existingCount > 0) {
      this.logger.warn(
        `   ⚠️  ${existingCount} positions already exist, skipping...`,
      );
      return;
    }
    ...
    this.logger.log(
      `   ✅ Seeded ${SeedDataConfig.positions.length} positions`,
    );
  ...
  async seedUsers(): Promise<void> {
    ...
    if (existingCount > 0) {
      this.logger.warn(
        `   ⚠️  ${existingCount} users already exist, skipping...`,
      );
      return;
    }
    ...
    const roles = {
      ...
      owner_company: SeedDataConfig.users.filter(
        (u) => u.role === 'owner_company',
      ).length,
      manager_company: SeedDataConfig.users.filter(
        (u) => u.role === 'manager_company',
      ).length,
      staff_company: SeedDataConfig.users.filter(
        (u) => u.role === 'staff_company',
      ).length,
      staff_unassigned: SeedDataConfig.users.filter(
        (u) => u.role === 'staff_unassigned',
      ).length,
    };
  ...
  async seedFormTemplates(): Promise<void> {
    ...
    if (existingCount > 0) {
      this.logger.warn(
        `   ⚠️  ${existingCount} form templates already exist, skipping...`,
      );
      return;
    }
    ...
    this.logger.log(
      `   ✅ Seeded ${SeedDataConfig.formTemplates.length} form templates`,
    );
  ...
  async seedServices(): Promise<void> {
    ...
    if (existingCount > 0) {
      this.logger.warn(
        `   ⚠️  ${existingCount} services already exist, skipping...`,
      );
      return;
    }
    ...
    SeedDataConfig.services.forEach((s) => {
      this.logger.log(
        `      - ${s.title} (${s.workOrderForms.length} work order forms)`,
      );
    });
  }
  ...
  async seedMembershipCodes(): Promise<void> {
    ...
    if (existingCount > 0) {
      this.logger.warn(
        `   ⚠️  ${existingCount} membership codes already exist, skipping...`,
      );
      return;
    }
    ...
    this.logger.log(
      `   ✅ Seeded ${SeedDataConfig.membershipCodes.length} membership codes`,
    );
    SeedDataConfig.membershipCodes.forEach((m) => {
      this.logger.log(
        `      - ${m.code} (${m.isClaimed ? 'claimed' : 'available'})`,
      );
    });
  }
  ...
  async getStatus(): Promise<any> {
    ...
    const totalSeeded =
      ...
      formTemplatesCount +
      servicesCount +
      membershipCodesCount;
    const totalExpected =
      SeedDataConfig.companies.length +
    ...
    return {
      ...
      formTemplates: {
        ...
        percentage:
          (formTemplatesCount / SeedDataConfig.formTemplates.length) * 100,
      },
      services: {
      ...
      membershipCodes: {
        ...
        percentage:
          (membershipCodesCount / SeedDataConfig.membershipCodes.length) * 100,
      },
    };
  }
}
