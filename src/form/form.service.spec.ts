...
describe('FormsService', () => {
    ...
    describe('submitForm()', () => {
        ...
        it('UT-FRM-001: should submit form with all valid fields', async () => {
            ...
            jest.spyOn(companiesService, 'findInternalById').mockResolvedValue(mockCompany as any);
            jest.spyOn(formValidationHelper, 'validateFormSubmission').mockImplementation(() => { });
        ...
        it('UT-FRM-002: should validate text field correctly', async () => {
            ...
            formTemplateModel.findOne.mockReturnValue({ exec: mockExec });
            jest.spyOn(companiesService, 'findInternalById').mockResolvedValue(mockCompany as any);
            const validateSpy = jest.spyOn(formValidationHelper, 'validateFormSubmission').mockImplementation(() => { });
            ...
            // Assert
            expect(validateSpy).toHaveBeenCalledWith(templateWithTextField.fields, submissionDto.fieldsData);
        });
        ...
        it('UT-FRM-003: should validate number field correctly', async () => {
            ...
            // Assert
            expect(validateSpy).toHaveBeenCalledWith(templateWithNumberField.fields, submissionDto.fieldsData);
        });
        ...
        it('UT-FRM-004: should throw BadRequestException for invalid type on number field', async () => {
            ...
            // Act & Assert
            await expect(service.submitForm(mockUser as any, submissionDto)).rejects.toThrow(BadRequestException);
        });
        ...
        it('UT-FRM-006: should validate single_select with valid option', async () => {
            ...
            // Assert
            expect(validateSpy).toHaveBeenCalledWith(templateWithSelect.fields, submissionDto.fieldsData);
        });
        ...
        it('UT-FRM-008: should validate multi_select with valid options', async () => {
            ...
            // Assert
            expect(validateSpy).toHaveBeenCalledWith(templateWithMultiSelect.fields, submissionDto.fieldsData);
        });
