...
describe('InvitationsService', () => {
    ...
    describe('acceptInvitation()', () => {
        ...
        it('UT-INV-001: should accept invitation successfully', async () => {
            ...
            const mockSelect = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(updatedUser) });
            ...
            // Assert
            expect(invitationModel.findById).toHaveBeenCalledWith('507f1f77bcf86cd799439020');
        ...
        it('UT-INV-009: should change invitation status to accepted', async () => {
            ...
            const mockSelect = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(mockUserDocument) });
            userModel.findByIdAndUpdate.mockReturnValue({ select: mockSelect });

            const saveSpy = jest.spyOn(mockInvitation, 'save');

            // Act
            await service.acceptInvitation('507f1f77bcf86cd799439020', mockUser as any);
        ...
        it('UT-INV-010: should save both user and invitation', async () => {
            ...
            const mockSelect = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(updatedUser) });
            const updateSpy = jest.spyOn(userModel, 'findByIdAndUpdate').mockReturnValue({ select: mockSelect });
