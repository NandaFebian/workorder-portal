...
describe('WorkOrderService', () => {
    ...
    describe('updateStatus()', () => {
        ...
        it('UT-WO-001: should update status from drafted to ready', async () => {
            ...
            jest.spyOn(service, 'findOneInternal').mockResolvedValue(wo as any);

            // Act
            const result = await service.updateStatus('507f1f77bcf86cd799439013', { status: 'ready' }, mockUser as any);
        ...
        it('UT-WO-002: should update status to in_progress and set startedAt', async () => {
            ...
            // Act
            await service.updateStatus('507f1f77bcf86cd799439013', { status: 'in_progress' }, mockUser as any);

            // Assert
            expect(wo.status).toBe('in_progress');
            expect(wo.startedAt).toBeInstanceOf(Date);
            expect(wo.startedAt!.getTime()).toBeGreaterThanOrEqual(beforeTime.getTime());
            expect(wo.save).toHaveBeenCalled();
        });
        ...
        it('UT-WO-003: should update status to completed and set completedAt', async () => {
            ...
            // Act
            await service.updateStatus('507f1f77bcf86cd799439013', { status: 'completed' }, mockUser as any);

            // Assert
            expect(wo.status).toBe('completed');
            expect(wo.completedAt).toBeInstanceOf(Date);
            expect(wo.completedAt!.getTime()).toBeGreaterThanOrEqual(beforeTime.getTime());
            expect(wo.save).toHaveBeenCalled();
        });
        ...
        it('UT-WO-005: should allow invalid transition from completed to ready (no validation)', async () => {
            ...
            // Act
            await service.updateStatus('507f1f77bcf86cd799439013', { status: 'ready' }, mockUser as any);
        ...
        it('UT-WO-008: should throw NotFoundException when work order not found', async () => {
            ...
            // Act & Assert
            await expect(
                service.updateStatus('507f1f77bcf86cd799439013', { status: 'ready' }, mockUser as any),
        ...
        it('UT-WO-009: should throw BadRequestException when user company info is missing', async () => {
            ...
            // Act & Assert
            await expect(
                service.updateStatus('507f1f77bcf86cd799439013', { status: 'ready' }, userWithoutCompany as any),
    ...
    describe('assignStaff()', () => {
        ...
        it('UT-WO-010: should assign staff successfully for owner role', async () => {
            ...
            jest.spyOn(usersService, 'findOneByEmail').mockResolvedValue(mockStaff as any);
            jest.spyOn(service, 'findOneInternal').mockResolvedValue(mockWorkOrder as any);
            ...
            // Assert
            expect(usersService.findOneByEmail).toHaveBeenCalledWith('staff@test.com');
        ...
        it('UT-WO-013: should throw BadRequestException when staff is from different company', async () => {
            ...
            workOrderModel.findOne.mockResolvedValue(mockWorkOrder);
            jest.spyOn(usersService, 'findOneByEmail').mockResolvedValue(staffFromDifferentCompany as any);
        ...
        it('UT-WO-014: should handle empty staffIds array', async () => {
            ...
            // Act
            await service.assignStaff('507f1f77bcf86cd799439013', { staffEmail: [] }, mockUser as any);
        ...
        it('UT-WO-015: should assign multiple staff members', async () => {
            // Arrange
            const staff1 = { ...mockStaff, _id: '507f1f77bcf86cd799439020', email: 'staff1@test.com' };
            const staff2 = { ...mockStaff, _id: '507f1f77bcf86cd799439021', email: 'staff2@test.com' };
            const staff3 = { ...mockStaff, _id: '507f1f77bcf86cd799439022', email: 'staff3@test.com' };
        ...
        it('UT-WO-016: should validate each staff email during assignment', async () => {
            // Arrange
            workOrderModel.findOne.mockResolvedValue(mockWorkOrder);
            const findByEmailSpy = jest.spyOn(usersService, 'findOneByEmail').mockResolvedValue(mockStaff as any);
            jest.spyOn(service, 'findOneInternal').mockResolvedValue(mockWorkOrder as any);
