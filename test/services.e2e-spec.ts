...
describe('ServicesController (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    const regRes = await request(app.getHttpServer())
      ...
      .send({ name: 'SVC Owner', email: 'svcowner@test.com', password: 'password123', companyName: 'SVC Corp' })
      .expect(200);
    ...
    const intakeRes = await request(app.getHttpServer())
      ...
      .send({
        ...
        formType: 'intake',
        fields: [{ order: 1, label: 'Nama Pemohon', type: 'text', required: true }],
      })
      .expect(201);
    ...
    const reportRes = await request(app.getHttpServer())
      ...
      .send({
        ...
        formType: 'report',
        fields: [{ order: 1, label: 'Hasil Kerja', type: 'textarea', required: true }],
      })
      .expect(201);
  ...
  describe('POST /services', () => {
    it('[TC-SVC-01] should create a new service and return 201 (Blackbox)', async () => {
      ...
      expect(res.body.data).toEqual(
        expect.objectContaining({ title: 'Layanan Test', accessType: 'internal' }),
      );
    });
  ...
  describe('GET /services', () => {
    it('[TC-SVC-03] should return 200 OK with all services (Blackbox)', async () => {
      await createService().then((r) => { expect(r.status).toBe(201); });
    ...
    it('[TC-SVC-04] should only return services belonging to owner company (Whitebox)', async () => {
      ...
      const regB = await request(app.getHttpServer())
        .post('/auth/register-company')
        .send({ name: 'Owner B', email: 'ownerb@svc.com', password: 'password123', companyName: 'Company B' })
        .expect(200);
  ...
  describe('GET /public/services/company/:companyId', () => {
    it('[TC-SVC-17] should return public services without auth (Blackbox)', async () => {
      ...
      // Toggle active
      const services = await request(app.getHttpServer()).get('/services').set('Authorization', ownerToken).expect(200);
      const svcId = services.body.data[0]._id;
      await request(app.getHttpServer()).patch(`/services/${svcId}/toggle-active`).set('Authorization', ownerToken).send({ isActive: true });
